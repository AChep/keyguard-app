 ...
 * over a Unix domain socket (macOS/Linux).
 *
 * The socket lifecycle (bind, permissions, accept loop, connection
...
internal class AgentIpcServer(
    ...
    private val session: suspend (SocketChannel) -> Unit,
) {
    ...
    suspend fun start(
        ...
        if (osName.startsWith("Windows", ignoreCase = true)) {
            val msg = "Agent IPC server requires Unix domain sockets; " +
