...
/**
 * Cross-platform SSH-signing parity check for the JVM (BouncyCastle) signer.
 *
 * The Apple (CryptoKit) side asserts the same fixed key + message in
 * `macosApp/macosAppTests/SshSignerCoreTests.swift`. CryptoKit randomizes its Ed25519
 * signatures while BouncyCastle is deterministic, so the two stacks never produce
 * identical bytes — "the same" means *interoperable*:
 *   - same algorithm string ("ssh-ed25519"),
 *   - same RAW 64-byte signature format (the Rust agent wire-wraps it),
 *   - both signatures verify under the same public key.
 *
 * Being deterministic, the JVM side can pin the exact RFC-8032 signature. [EXPECTED_SIG_HEX]
 * was generated by an independent reference (Python `cryptography`); the Apple test feeds
 * the same value into Apple's verifier to prove interop. Keep these constants byte-identical
 * to the Swift test.
 */
class SshSigningParityTest {
    @Test
    fun `signWithPrivateKey reproduces the deterministic Ed25519 known-answer vector`() {
        val result = SshAgentRequestProcessorJvm.signWithPrivateKey(
            privateKeyPem = ED_PRIVATE_KEY_PEM,
            data = MESSAGE.encodeToByteArray(),
            flags = 0,
        )

        assertEquals("ssh-ed25519", result.algorithm)
        assertEquals(64, result.signature.size, "Ed25519 signatures must be raw 64 bytes (not wire-wrapped)")
        assertEquals(EXPECTED_SIG_HEX, result.signature.toHex())
    }

    @Test
    fun `signWithPrivateKey signature verifies under the public key`() {
        val result = SshAgentRequestProcessorJvm.signWithPrivateKey(
            privateKeyPem = ED_PRIVATE_KEY_PEM,
            data = MESSAGE.encodeToByteArray(),
            flags = 0,
        ...
        val publicKey = Ed25519PublicKeyParameters(hex(ED_PUBLIC_KEY_HEX), 0)
        val verifier = Ed25519Signer()
        verifier.init(false, publicKey)
        val data = MESSAGE.encodeToByteArray()
        verifier.update(data, 0, data.size)
        assertTrue(verifier.verifySignature(result.signature), "JVM signature should verify")
    }

    // ================================================================
    // RSA — PKCS#1 v1.5 is deterministic, so the Apple (Security framework)
    // signer must produce these exact bytes too. The expected signatures were
    // generated independently with `openssl dgst -<hash> -sign`.
    // ================================================================

    @Test
    fun `signWithPrivateKey reproduces RSA known-answer vectors for PKCS1 key`() {
        assertRsaParity(RSA_PKCS1_PRIVATE_KEY_PEM)
    }

    @Test
    fun `signWithPrivateKey reproduces RSA known-answer vectors for openssh-key-v1 key`() {
    ...
    private companion object {
        ...
        const val ED_PUBLIC_KEY_HEX = "b8e2b9a67d07f9f1b983ff7c50a9e7d9c93f74b4471c63935558035f052d753f"

        const val EXPECTED_SIG_HEX =
            "5b5fbfdad533567a8a7a585eeb2d96d4a0ae2032edad4e90211c802c50eddf8f" +
                "0865f7081b5c4d334388166dc047d28efaf79a1f6802f4d12f03603e122cb805"

        // The same openssh-key-v1 blob pinned in KeyPairGeneratorAppleTest, wrapped in PEM.
        ...
        val ED_PRIVATE_KEY_PEM =
            "-----BEGIN OPENSSH PRIVATE KEY-----\n$ED_PRIV_BLOB_B64\n-----END OPENSSH PRIVATE KEY-----\n"

        // A fixed RSA-2048 key in both stored formats (ssh-keygen). The same key + message
        // is asserted by the Apple RSA signer test; keep these constants byte-identical.
