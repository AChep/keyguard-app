...
/**
 * Cross-platform SSH-signing parity check for the production native signer.
 *
 * The macOS native-library test asserts the same fixed key and message. "The same" means:
 ...
 *
 * Ed25519 is deterministic, so the JVM side pins the exact RFC-8032 signature. [EXPECTED_SIG_HEX]
 * was generated by an independent reference (Python `cryptography`); the Apple test feeds
 * the same value into an independent verifier to prove interoperability.
 */
class SshSigningParityTest {
    ...
    // ================================================================
    // RSA — PKCS#1 v1.5 is deterministic, so the shared native signer on Apple
    // must produce these exact bytes too. The expected signatures were
    ...
    private companion object {
        ...
        // The same openssh-key-v1 blob pinned in NativeKeyPairGeneratorTest, wrapped in PEM.
