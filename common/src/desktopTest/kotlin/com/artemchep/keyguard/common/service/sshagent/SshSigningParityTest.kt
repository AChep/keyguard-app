...
class SshSigningParityTest {
    @Test
    fun `native sign reproduces the deterministic Ed25519 known-answer vector`() {
    ...
    @Test
    fun `native sign signature verifies under the public key`() {
        val result = NativeCrypto.ssh.sign(
            privateKeyPem = ED_PRIVATE_KEY_PEM,
            publicKeyOpenSsh = null,
            data = MESSAGE.encodeToByteArray(),
    ...
    @Test
    fun `native sign reproduces RSA known-answer vectors for PKCS1 key`() {
        assertRsaParity(RSA_PKCS1_PRIVATE_KEY_PEM)
    }

    @Test
    fun `native sign reproduces RSA known-answer vectors for openssh-key-v1 key`() {
