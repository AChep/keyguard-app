            ...
            try {
                createHmacSha256(keys.macKey).use { hmac ->
                    ...
                    SystemFileSystem.sink(outputPath)
                        .buffered()
                        .use { sink ->
                            val inputBuffer = ByteArray(FileEncryptionFormat.BUFFER_SIZE)
                            val outputBuffer = ByteArray(
                                FileEncryptionFormat.BUFFER_SIZE + kCCBlockSizeAES128.toInt(),
                            )
