...
class GpgOpenPgpServiceJvm() : GpgOpenPgpService {
    constructor(
        directDI: DirectDI,
    ) : this()
    ...
    override fun verifyClearSignedText(
        ...
        if (publicKey == null) {
            return missingPublicKey(signature)
        }
    ...
    private fun updateCanonicalTextSignature(
    ...
    private fun readSignatureList(
        ...
        var obj = JcaPGPObjectFactory(input).nextObject()
        if (obj is PGPCompressedData) {
            obj = JcaPGPObjectFactory(obj.dataStream).nextObject()
        }
        return obj as? PGPSignatureList
    ...
    // A message may carry several signatures; verify against the first one we actually
    // hold a public key for. When none resolves, fall back to the first signature so the
    // MISSING_PUBLIC_KEY report still describes a real signer (single-signature behavior
    // is unchanged — the first signature is the only one).
