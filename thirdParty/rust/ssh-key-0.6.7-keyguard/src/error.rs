...
impl From<encoding::Error> for Error {
    fn from(err: encoding::Error) -> Error {
        match err {
            // Keyguard compatibility extension: all SSH format labels are
            // algorithm identifiers, so preserve the typed distinction
            // between an unknown algorithm and malformed framing.
            encoding::Error::Label(_) => Error::AlgorithmUnknown,
