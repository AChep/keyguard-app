...
#![cfg_attr(
    all(
        any(feature = "encryption", feature = "keyguard-decryption"),
        feature = "std"
    ),
    doc = " ```"
)]
...
impl PrivateKey {
    ...
    /// Create a new unencrypted private key with an explicit duplicated
    /// OpenSSH checkint.
    ///
    /// This is a Keyguard compatibility extension. Historical Keyguard keys
    /// use a fresh random checkint, while the upstream constructor derives a
    /// deterministic value when encoding newly assembled key data.
    #[cfg(feature = "alloc")]
    pub fn new_with_checkint(
        key_data: KeypairData,
        comment: impl Into<String>,
        checkint: u32,
    ) -> Result<Self> {
        let mut private_key = Self::new(key_data, comment)?;
        private_key.checkint = Some(checkint);
        Ok(private_key)
    }
    ...
    /// Parse an OpenSSH-formatted PEM private key while deriving the public
    /// key from the inner private record instead of requiring the duplicated
    /// outer public record to match.
    ///
    /// This is a Keyguard import-compatibility extension for SSHJ 0.40.0,
    /// which treated the outer record as metadata. Callers must use the
    /// regular [`Self::from_openssh`] API when strict OpenSSH validation is
    /// required.
    pub fn from_openssh_without_public_key_check(pem: impl AsRef<[u8]>) -> Result<Self> {
        const OPENSSH_PEM_LINE_WIDTH: usize = 70;
        let mut reader = encoding::pem::Decoder::new_wrapped(pem.as_ref(), OPENSSH_PEM_LINE_WIDTH)
            .map_err(Error::from)?;
        Self::validate_pem_label(reader.type_label()).map_err(Error::from)?;
        let private_key = Self::decode_with_public_key_check(&mut reader, false)?;
    ...
    /// Parse raw OpenSSH private-key bytes while deriving the public key from
    /// the inner private record instead of requiring the duplicated outer
    /// public record to match.
    ///
    /// This is the raw counterpart to
    /// [`Self::from_openssh_without_public_key_check`]. It lets import callers
    /// decode sensitive PEM armor directly into zeroizing storage instead of
    /// asking the PEM decoder to allocate another private-key buffer.
    pub fn from_bytes_without_public_key_check(mut bytes: &[u8]) -> Result<Self> {
        let reader = &mut bytes;
        let private_key = Self::decode_with_public_key_check(reader, false)?;
        Ok(reader.finish(private_key)?)
    }
    ...
    pub fn decrypt(&self, password: impl AsRef<[u8]>) -> Result<Self> {
        let key_and_iv = self.kdf.derive_key_and_iv(self.cipher, password)?;
    ...
    /// Decrypt an OpenSSH key while deriving the public key from the inner
    /// private record instead of requiring the duplicated outer public record
    /// to match.
    ///
    /// This is a Keyguard import-compatibility extension for SSHJ 0.40.0.
    #[cfg(any(feature = "encryption", feature = "keyguard-decryption"))]
    pub fn decrypt_without_public_key_check(&self, password: impl AsRef<[u8]>) -> Result<Self> {
        let key_and_iv = self.kdf.derive_key_and_iv(self.cipher, password)?;
        let ciphertext = self.key_data.encrypted().ok_or(Error::Decrypted)?;
        let mut buffer = Zeroizing::new(ciphertext.to_vec());
        self.cipher.decrypt(
    ...
    pub fn encrypt_with(
        ...
        let key_and_iv = kdf.derive_key_and_iv(cipher, password)?;
        ...
        let auth_tag = cipher.encrypt(key_and_iv.key(), key_and_iv.iv(), out.as_mut_slice())?;
    ...
    fn decode_privatekey_comment_pair(
        ...
        let inner_public_key = public::KeyData::try_from(&key_data)?;
        if check_public_key && public_key != inner_public_key {
            return Err(Error::PublicKey);
        }

        let mut public_key = PublicKey::from(inner_public_key);
        public_key.decode_comment(reader)?;

        let padding_len = reader.remaining_len();
        if check_public_key && padding_len >= block_size {
            return Err(encoding::Error::Length.into());
        }
    ...
    fn decode_with_public_key_check(
        reader: &mut impl Reader,
        check_public_key: bool,
    ) -> Result<Self> {
...
impl Decode for PrivateKey {
    ...
    fn decode(reader: &mut impl Reader) -> Result<Self> {
        Self::decode_with_public_key_check(reader, true)
    }
}
