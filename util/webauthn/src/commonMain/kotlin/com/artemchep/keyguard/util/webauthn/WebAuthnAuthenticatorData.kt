...
class WebAuthnAuthenticatorDataFactory(
    ...
    private val hashMd5: (ByteArray) -> ByteArray = NativeCrypto.primitives::md5,
) {
    private val aaguid = aaguid.copyOf().also {
        require(it.size == AAGUID_SIZE_BYTES) { "WebAuthn AAGUID must contain 16 bytes." }
    }
    ...
    fun encodeAuthenticatorData(
        ...
        val rpIdHash = hashSha256(rpId.encodeToByteArray())
    ...
    private fun aaguidFor(
        ...
        CreatePasskeyAttestation.INDIRECT -> {
            val hash = hashMd5(aaguid + credentialId)
